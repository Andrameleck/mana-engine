...
tryCatch({
  ...
  catalog <- if (is.list(payload$cards) && length(payload$cards) > 0L) {
    ...
    catalog_out <- call_query("query_synergy_get_catalog", force_refresh = isTRUE(payload$force_refresh))
    if (!isTRUE(catalog_out$ok)) {
      stop(trimws(as.character(catalog_out$error %||% "catalog unavailable")))
    }
    catalog_out$cards
  }
